function [net,stats] = cnn_train_dag_custom(net, getBatch, varargin)
...
% Slightly modified by Ignacio Rocco
...
for p=1:numel(net.params)
  ...
  % added by Ignacio: skip update if learning rate is zero for this
  % parameter
  if net.params(p).learningRate==0 
      continue;
  end
...
% -------------------------------------------------------------------------
% Assumes the last layer of the net is the loss, and that the Loss object
% has a 'loss' property (which is the case for dagnnExtra.AffineLoss, and
% dagnnExtra.LossDist2)
stats.loss = gather(mean(net.layers(end).block.loss));
