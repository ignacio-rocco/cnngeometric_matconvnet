function results = evaluatePropFlow(paths,varargin)
...
evalopts.affnet = [];
evalopts.affnet2 = [];
evalopts.resultname = 'eval.mat';
evalopts.useGPU=0;

% override with user provided parameters
evalopts = vl_argparse(evalopts,varargin);

% check if affine ensemble is specified and adjust resultname
...
%% Load dataset pairs
load(fullfile(pfPath,'test_pairs_pf.mat')) ; % load pair list
...
%% Compute PCK for all dataset
pckVec = zeros(Npairs,1);

% allocate space for storing affine transformations
if doAffine==1
...
% allocate space for storing TPS transformations
if doTps==1
...
for idx=1:Npairs
   ...
   elseif doTps==1 && doAffine==0
    ...
    if false
        ...
        scatter(ptsBtpsaffX,ptsBtpsaffY,500,colors,'.');   

        figure(2); clf; imshow(uint8(imA_+128))
        figure(3); clf; imshow(uint8(imB_+128))
        % warp im
        tps = dagnn.TpsGridGenerator('k',k,'Wo',wB,'Ho',hB,'lambda',lambda);
        bs = dagnn.BilinearSampler();
        grid = tps.forward({thetaNN});
        imA_tps = bs.forward({single(imA),grid{1}})
        figure(4); clf; imshow(uint8(imA_tps{1}))    
    end
